...
int is_valid_string(char *inficks) {
  ...
  if (string_length(inficks) == 0) {
    ...
    int dot_count = 0;  // Счетчик для проверки кол-ва точек в числе
    for (int i = 0; inficks[i] != '\0'; i++) {
      ...
      if (inficks[i] == '.') {
        ...
        if (dot_count > 1) {
          res_is_valid_string =
              0;  // Если в числе более одной точки, строка недопустима
        }
      }
      if (inficks[i] == '+' || inficks[i] == '-' || inficks[i] == '*' ||
          inficks[i] == '/') {
        dot_count = 0;  // Сбрасываем счетчик точек при обнаружении нового числа
      }
